package com.example.mynavigator.ui.data;


public class CwData {

    public int cnctrLghtFcltyYn; // bool
    public int brllBlckYn;// bool
    public int tfcilndYn;// bool
    public int sondSgngnrYn;// bool
    public int tfclghtYn;// bool
    public int et;
    public int bt;
    public int cartrkCo;
    public float longitude;
    public float latitude;
    public int bcyclCrslkCmbnatYn;// bool
    public int crslkKnd;
    public String lnmadr;
    public String roadNm;
    public int ctprvnNm;


    public int isCnctrLghtFcltyYn() {
        return cnctrLghtFcltyYn;
    }

    public void setCnctrLghtFcltyYn(int cnctrLghtFcltyYn) {
        this.cnctrLghtFcltyYn = cnctrLghtFcltyYn;
    }

    public int isBrllBlckYn() {
        return brllBlckYn;
    }

    public void setBrllBlckYn(int brllBlckYn) {
        this.brllBlckYn = brllBlckYn;
    }

    public int isTfcilndYn() {
        return tfcilndYn;
    }

    public void setTfcilndYn(int tfcilndYn) {
        this.tfcilndYn = tfcilndYn;
    }

    public int isSondSgngnrYn() {
        return sondSgngnrYn;
    }

    public void setSondSgngnrYn(int sondSgngnrYn) {
        this.sondSgngnrYn = sondSgngnrYn;
    }

    public int isTfclghtYn() {
        return tfclghtYn;
    }

    public void setTfclghtYn(int tfclghtYn) {
        this.tfclghtYn = tfclghtYn;
    }

    public int getEt() {
        return et;
    }

    public void setEt(int et) {
        this.et = et;
    }

    public int getBt() {
        return bt;
    }

    public void setBt(int bt) {
        this.bt = bt;
    }

    public int getCartrkCo() {
        return cartrkCo;
    }

    public void setCartrkCo(int cartrkCo) {
        this.cartrkCo = cartrkCo;
    }

    public float getLongitude() {
        return longitude;
    }

    public void setLongitude(float longitude) {
        this.longitude = longitude;
    }

    public float getLatitude() {
        return latitude;
    }

    public void setLatitude(float latitude) {
        this.latitude = latitude;
    }

    public int isBcyclCrslkCmbnatYn() {
        return bcyclCrslkCmbnatYn;
    }

    public void setBcyclCrslkCmbnatYn(int bcyclCrslkCmbnatYn) {
        this.bcyclCrslkCmbnatYn = bcyclCrslkCmbnatYn;
    }

    public int getCrslkKnd() {
        return crslkKnd;
    }

    public void setCrslkKnd(int crslkKnd) {
        this.crslkKnd = crslkKnd;
    }

    public String getLnmadr() {
        return lnmadr;
    }

    public void setLnmadr(String lnmadr) {
        this.lnmadr = lnmadr;
    }

    public String getRoadNm() {
        return roadNm;
    }

    public void setRoadNm(String roadNm) {
        this.roadNm = roadNm;
    }

    public int getCtprvnNm() {
        return ctprvnNm;
    }

    public void setCtprvnNm(int ctprvnNm) {
        this.ctprvnNm = ctprvnNm;
    }





}
